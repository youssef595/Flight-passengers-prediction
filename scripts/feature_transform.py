...
def get_distance(X):
    url = ('https://raw.githubusercontent.com/ravisurdhar/'
           'flight_delays/master/airports.csv')
    us_airports = pd.read_csv(url)
    us_airports['coordinate_from'] = list(
        zip(us_airports.LATITUDE, us_airports.LONGITUDE))
    ...
    data = data.rename(
        columns={
            'coordinate_from_x': 'coordinate_from',
            'coordinate_to_y': 'coordinate_to'})
    data = data.drop(labels=['coordinate_to_x', 'coordinate_from_y'], axis=1)
    list_coordinates = list(
        zip(data['coordinate_from'].tolist(), data['coordinate_to'].tolist()))
    dist = []
    for i in range(len(list_coordinates)):
        dist.append(
            hs.haversine(
                list_coordinates[i][0],
                list_coordinates[i][1],
...
def scrap_weather(X, start, end):
    ...
    iatas_repeated = [
        iata for iata in iatas for i in range(
            list_data[0].shape[0])]
...
def merge_weather_data_1(X, weather_data):
    X = X.copy()
    weather_data = weather_data[['time', 'tavg',
                                 'prcp', 'wspd', 'airport_code', 'snow']]
...
def merge_weather_data_2(X):
    filepath = ('https://raw.githubusercontent.com/ramp-kits/air_passengers/'
                'master/submissions/use_external_data/external_data.csv')
    # Make sure that DateOfDeparture is of dtype datetime
    X = X.copy()  # modify a copy of X
    X.loc[:, "flight_date"] = pd.to_datetime(X['flight_date'])
    # Parse date to also be of dtype datetime
    data_weather = pd.read_csv(filepath, parse_dates=["Date"])
    cols_to_drop = data_weather.filter(like='Mean').columns.tolist(
    ) + data_weather.filter(like='Min').columns.tolist() + ['CloudCover']
    ...
    X_weather = X_weather.rename(
        columns={
            ...
            'AirPort': 'to'})
    X_merged = pd.merge(
        X, X_weather, how='left', on=[
            'flight_date', 'to'], sort=False)
    return X_merged.drop(['Precipitationmm', 'Events',
                         'WindDirDegrees', 'Max Gust SpeedKm/h'], 1)
